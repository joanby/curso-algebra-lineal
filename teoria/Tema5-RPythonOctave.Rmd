...
## Calcular Distancia Eucídea entre dos vectores con `R`

Para calcular la distancia entre dos vectores, definida como $\text{d}(x,y) = ||x-y||$ utilizamos la función `distancia()`, la cual definimos del siguiente modo:
...
distancia = function(x,y){
  if (length(x) == length(y)){
    ...
    print("ERROR: No se puede calcular la distancia entre estos dos vectores porque 
          no son de la misma dimensión")
  }
}
```
...
Simplemente, lo que hace la función anterior es calcular la distancia Euclídea entre dos vectores, siempre y cuando estos tengan la misma dimensión. 
...
productoMixto = function(x,y,z){
  if (length(x) == length(y) & length(x) == length(z) & length(x) == 3){
    ...
    print("No se cumplen las condiciones necesarias para calcular el 
          producto mixto de estos dos vectores")
  }
}
```
...
## Calcular Distancia Eucídea entre dos vectores con `Python`

Para calcular la distancia entre dos vectores, definida como $d(x,y)=||x−y||$ utilizamos la función `distancia()`, la cual definimos del siguiente modo:
