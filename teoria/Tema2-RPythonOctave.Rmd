...
Con lo cual, efectivamente $x=(1,2,3)$ es la solución de nuestros sistema. Además es la única, ya que recordemos que nuestro sistema era compatible determinado.
...
$$\{x\in \mathbb{R}^3 \ :\  x_1 = \frac{3}{2},\ x_2 = \frac{1}{2}+x_3,\ x_3\in\mathbb{R}\}$$
...
```{python}
linsolve(Matrix(([2, 2, 1], [-1, 1, 2])), (x, y))
```
...
Solamente mostraremos el código. Todo lo que se haga a continuación en esta sección conviene ejecutarlo en alguna plataforma como por ejemplo Jupyter.
...
## Sistemas compatibles indeterminados con `Python` {.exercise}

**Ejercicio**

- Comprobad haciendo uso del Teorema de Rouché-Frobenius que efectivamente se trata de un sistema compatible indeterminado.
- Repetid el proceso anterior, demostrando primero el tipo de sistema del cual se trata, para el sistema de dos ecuaciones con 3 incógnitas siguiente:

$$\left\{\begin{matrix}x+y+z = -1\\ x + y + 2z = -3\end{matrix}\right.$$
...
```
Esto se debe a que el sistema es incompatible.

<div class = "exercise">**Ejercicio.** Demostrar, utilizando el Teorema de Rouché-Frobenius y `Python`, que efectivamente se trata de un sistema incompatible</div>
