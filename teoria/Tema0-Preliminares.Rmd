...
Observad que para trabajar con los números complejos anteriormente definidos, hemos tenido que crearlos de nuevo porque Octave trata cada chunk por separado.
...
$p(x) = 1+x^2\in\mathbb{R}[x]$ es primo ya que no puede escribirse de forma $r(x)s(x)$ con $r(x),s(x)\in\mathbb{R}[x]$, $r(x),s(x)\ne p(x)$ y $r(x),s(x)\ne 1$.
...
Claramente son diferentes, porque, tal y como los hemos definido anteriormente, ni siquiera tienen el mismo grado. Además, fijaos que no podemos comparar los polinomios creados utilizando librerías diferentes, ya que da error. 
...
Fijaos que hemos tenido que volver a definir los polinomios. De no estar, la consola nos devolvería error.
