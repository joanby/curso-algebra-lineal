...
<l class = "prop">Caracterización de un vector fijo (I).</l> En el contexto geométrico, las 3 características de un vector fijo son:
...
Un vector lubre no tiene un origen fijo, sino que se puede ubicar en cualquier punto del espacio.
...
<l class = "prop">Ley de composición interna.</l> La suma de vectores se denomina ley de composición interna ya que opera entre elementos de un conjunto dado, $\mathbb{K}^n$ y el resultado es otro elemento de este conjunto

$$\begin{matrix}f:\mathbb{K}^n\times \mathbb{K}^n&\longrightarrow& \mathbb{K}^n\\ (\vec{u},\vec{v})&\mapsto&\vec{u}+\vec{v}\end{matrix}$$

## Propiedades de las operaciones con vectores

<l class = "prop">Ley de composición externa.</l> El producto de un escalar por un vector tiene como operandos conjuntos diferentes: escalares por un lado y vectores por el otro. El resultado cae del lado de los vectores y la operación se denomina ley de composición externa:
...
<l class = "prop">Propiedades de la suma de vectores.</l> Sean $\vec{u},\vec{v}\in\mathbb{K}^n$ y $\alpha,\beta\in\mathbb{K}$
...
$$||\vec{u}|| = \sqrt{\langle\vec{u},\vec{u}\rangle} = \sqrt{\vec{u}_1^2+\vec{u}_2^2+\cdots+\vec{u}_n^2}$$
...
- Teorema de Pitágoras: $||\vec{u}+\vec{v}|| = ||\vec{u}||+||\vec{v}||\Leftrightarrow \vec{u}\perp\vec{v}$
- Desigualdad de Cauchy-Schwarz: $||\vec{u}\vec{v}||\le||\vec{u}||\cdot||\vec{v}||$
...
## Desigualdad de Cauchy-Schwarz{.example}
