...
Para llamar a una columna, necesitamos recorrer todas las filas de la matriz:
...
No obstante, hay una forma más sencilla: 
...
Para crear una matriz de ceros, utilizamos la función `np.ones((fil,col))`:
...
## Determinante, rango e inversa en Python
...
Fijaos que la matriz $M$ tiene determinante 0, lo que se traduce en que no tiene inversa. Sin embargo, Python la calcula sin queja alguna porque utiliza métodos numéricos. No obstante, el resultado es una «matriz infinita» que no tiene mucho sentido:
...
## Determinante, rango e inversa en Octave

Para calcular el determinante, utilizamos la función `det)()`:
...
Ocurrirá lo mismo que en Python: por mucho que la matriz no sea invertible, Octave nos calculará la inversa numéricamente. En cambio, en este caso, nos devolverá un «warning» indicando que la matriz es singular.
